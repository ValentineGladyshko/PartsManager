...
{
    public partial class MyInvoiceWindow : Window, INotifyPropertyChanged
    {
        ...
        public MyInvoiceWindow(bool isMine)
        {
            InitializeComponent();
            if (isMine)
            {
        ...
        public MyInvoiceWindow(Invoice invoice)
        {
            InitializeComponent();
        ...
        private void ChangeButtons(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (e.PropertyName == "PartPrices")
            {
                foreach (var part in LocalInvoice.InvoiceParts)
                {
                    part.DeliveryPrice = Math.Round(LocalInvoice.DeliveryPrice * (part.SumOut / LocalInvoice.SumOut) / part.Count, 2) * part.Count;
                    unitOfWork.InvoiceParts.Update(part);
                }
                var PartsDeliveryPrice = LocalInvoice.InvoiceParts.Sum(item => item.DeliveryPrice);
                if (PartsDeliveryPrice != LocalInvoice.DeliveryPrice)
                {
                    var part = LocalInvoice.InvoiceParts.ToList().Find(item => item.Count == 1);
                    if (part != null)
                    {
                        part.DeliveryPrice -= (PartsDeliveryPrice - LocalInvoice.DeliveryPrice);
                        unitOfWork.InvoiceParts.Update(part);
                    }
                    else 
                    {
                        LocalInvoice.DeliveryPrice = PartsDeliveryPrice;
                        InvoiceDeliveryPriceBox.GetBindingExpression(TextBox.TextProperty).UpdateTarget();
        ...
        public void SetHandlers()
        {
            ...
            WorkInvoicePartButton.Click += delegate
            {
                if (!IsPartEditing)
                {
                    ...
                    {
                        if (LocalInvoicePart.PriceOut > (LocalInvoicePart.RecommendedPrice2 * 2) || LocalInvoicePart.PriceOut < LocalInvoicePart.PriceIn)
                        {
                            string message = $"Ціна продажу: {LocalInvoicePart.PriceOut:C2} \nзначно більша за рекомендовану: {LocalInvoicePart.RecommendedPrice2:C2}";
